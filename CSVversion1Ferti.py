...
datos_geo = gpd.GeoDataFrame(datos, geometry='geometry', crs="EPSG:4326")
datos_geo['intensidad'] = (datos_geo['dosis1'] + datos_geo['dosis2']) / 2
datos_geo['velocidad'] = pd.to_numeric(datos_geo['Velocidad[Km/h]'], errors='coerce')
...
def clasificar_dosis_segun_diseno(grilla, valores_diseño):
    ...
    for dosis in grilla['dosis']:
        # MENOR AL DISEÑO → menor que el 90% del mínimo
        if dosis < min_d * 0.90:
            etiquetas.append("Menor al diseño")
        
        # MAYOR AL DISEÑO → mayor que el 110% del máximo
        elif dosis > max_d * 1.10:
            etiquetas.append("Mayor al diseño")
        
        # Dentro del rango de diseño (con tolerancia ±5%)
        else:
            asignado = False
            for i in range(len(valores) - 1):
                lim_inf = valores[i] * 0.95
                lim_sup = valores[i + 1] * 1.05
                if lim_inf <= dosis < lim_sup:
                    etiquetas.append(f"{int(valores[i])}")
                    asignado = True
                    break
            # Si no quedó asignado en ningún intervalo, se asigna a la última clase
            if not asignado:
...
def generar_csv_masivo(output_csv_path):
    ...
    for pante1 in datos_geo['PANTE1'].dropna().unique():
        datos_pante = datos_geo[datos_geo['PANTE1'] == pante1]
...
pantes_unicos = datos_geo['PANTE1'].dropna().unique()
